...
public enum hEnvironment: Hashable {
    case production
    case staging
    ...
    public static func setCurrent(_ environment: hEnvironment) {
        UserDefaults.standard.set(environment.rawValue, forKey: targetEnvironmentKey)
    }

    public static var current: hEnvironment {
        guard
            let targetEnvirontmentRawValue = UserDefaults.standard.value(forKey: targetEnvironmentKey)
                as? String, let targetEnvironment = hEnvironment(rawValue: targetEnvirontmentRawValue)
        else {
