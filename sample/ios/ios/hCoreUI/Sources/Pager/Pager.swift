...
extension Pager: Presentable {
    public func materialize() -> (UIViewController, Future<Void>) {
        ...
        containerView.snp.makeConstraints { make in make.width.centerX.centerY.equalToSuperview()
        ...
        bag += containerView.addArranged(pager) { pagerView in
            pagerView.snp.makeConstraints { make in make.width.centerX.equalToSuperview() }
        }
        ...
        controlsWrapper.snp.makeConstraints { make in make.width.equalToSuperview() }

        let pagerDots = PagerDots()

        bag += controlsWrapper.addArranged(pagerDots) { pagerDotsView in
            pagerDotsView.snp.makeConstraints { make in make.centerX.equalToSuperview()
        ...
        return (
            viewController,
            Future { completion in bag += closeButton.onTapSignal.onValue { completion(.success) }
