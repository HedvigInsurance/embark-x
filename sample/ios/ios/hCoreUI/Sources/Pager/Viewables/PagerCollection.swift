    ...
    public static func makeAndConfigure() -> (make: UIView, configure: (PagerItem) -> Disposable) {
        ...
        return (
            ...
            { sliderPage in sliderPageView.subviews.forEach { view in view.removeFromSuperview() }
                ...
                contentScreen.view.snp.makeConstraints { make in make.width.height.equalToSuperview() }
...
extension PagerCollection: Viewable {
    func materialize(events _: ViewableEvents) -> (UICollectionView, Disposable) {
        ...
        bag += collectionKit.view.signal(for: \.contentOffset).compactMap { _ in collectionKit.currentIndex }
            .distinct().onValue { pageIndex in self.scrolledToPageIndexCallbacker.callAll(with: pageIndex) }
