...
extension PagerDots: Viewable {
    public func materialize(events _: ViewableEvents) -> (UIView, Disposable) {
        ...
        bag += view.didLayoutSignal.onValue {
            stackView.snp.makeConstraints { make in make.height.centerX.centerY.equalToSuperview() }
        }
        ...
        bag += pageAmountSignal.atOnce().filter { $0 != 0 }
            .onValue { pageAmount in for subview in stackView.subviews { subview.removeFromSuperview() }
                ...
                for i in 0...pageAmount - 1 {
                    ...
                    indicator.transform =
                        i == 0
                        ? CGAffineTransform(scaleX: 1.5, y: 1.5) : CGAffineTransform.identity
                    indicator.layer.cornerRadius = 2

                    indicator.snp.makeConstraints { make in make.width.height.equalTo(4) }
