...
public struct CampaignBubble { let campaignTypeSignal: ReadSignal<CampaignType?>
    ...
    public enum CampaignType {
        ...
        case percentageDiscount(value: Double, months: Int)
        case invited
        case monthlyDeduction(amount: String)
    }

    public init(campaignTypeSignal: ReadSignal<CampaignType?>) { self.campaignTypeSignal = campaignTypeSignal }
}
        ...
        bag += campaignTypeSignal.animated(style: SpringAnimationStyle.mediumBounce()) { value in
            if let value = value {
                ...
                case let .percentageDiscount(value, months):
                    ...
                    if months == 1 {
                        subtitlelabel.text =
                            L10n.offerScreenPercentageDiscountBubbleTitleSingular(
                                Int(value)
                            )
                    } else {
                        subtitlelabel.text =
                            L10n.offerScreenPercentageDiscountBubbleTitlePlural(
                                Int(value),
                                months
                            )
                    }
